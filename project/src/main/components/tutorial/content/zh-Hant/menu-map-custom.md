...
管理自己的自訂方言特徵資料，並將其繪製到地圖上。使用者的自訂資料僅本人可見，與帳號綁定，在不同裝置登入也能使用。

## 補充自訂資料

適用於補充檔案中缺少的方言點或特徵資料。例如檔案裡的東莞方言點資料有限，只有石龍一個點有流攝資料：
...
在地圖頁面的右側面板，可以新增自訂資料，此時會自動填寫好類別、當前查詢特徵：

![在 map/view 頁面右側面板新增自訂資料](/tutorial/menu-map-custom/05-view-panel-add.webp)

填寫流程：

1. **地點簡稱**：填入要補充的地點簡稱
2. **分區**：如果地點存在於檔案裡，會自動匹配填入分區；否則需手動填寫。推薦分區與音典檔案保持一致，方便後續搜尋（如填入「嶺南-珠江-莞寶」，以後輸入嶺南/珠江/莞寶都能搜到）
3. **座標**：點選地圖即可自動填入經緯度
4. **類別、當前查詢特徵**：根據頁面自動識別，無需手動填入
5. **值**：顯示在地圖上的值
6. **說明**（可選）：在地圖上點選自訂地點時彈窗顯示
