...
describe('vocabulary explore page shell wiring', () => {
  ...
  it('builds the vocabulary workspace with reusable controls and three workflows', () => {
    ...
    expect(vocabularyPage).toContain("import { getVocabularyItems, getVocabularyMapPoints, uploadVocabulary } from '@/api'")
  ...
  it('keeps vocabulary card and map queries separate from the table API', () => {
    ...
    expect(vocabularyPage).toContain("return activeWorkflow.value === 'list' && viewMode.value === 'card'")
    expect(vocabularyPage).toContain("return activeWorkflow.value === 'list' && viewMode.value === 'map'")
