...
6. 小地图支持选点、单点预览、多点预览
7. 保存/删除后视图状态正确更新
8. `getDataByFeature(feature, phonology)` 已按真实协议收紧为 phonology 必传语义
9. 模式1 / 模式2 已补齐本地搜索
10. 模式1新增地点、模式2地点记录编辑均已接入 `batchMatch()` 自动补全与 `getRegions()` 分区辅助填充
11. 模式1重复 `聲韻調 + 特徵`、模式2重复 `簡稱 + 音典分區` 已补齐保存前确认
12. 删除地点记录已补齐确认
13. 弹窗关闭后会重置模式并重建子视图状态
14. 保存按钮已补齐“保存中”禁重提交
15. 新弹窗相关新增文案已进入三语 i18n
16. 样式已进一步收敛到项目现有 glass surface 语义，优先复用 `main-glass-panel-inner` 与既有按钮/关闭按钮样式

仍可继续增强但不阻塞当前交付：

1. 若后端聚合接口稳定提供 `updated_at`，当前卡片已支持展示；若未来要更统一，可改为格式化后的相对时间/日期
2. 重复确认当前使用项目现有消息 + 原生 confirm；如后续要完全视觉统一，可升级为项目内统一确认弹层
