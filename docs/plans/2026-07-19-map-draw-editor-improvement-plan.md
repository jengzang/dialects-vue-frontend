...
| 选择能力补强 | 已完成 | `4525e921`、`011baf88`、`b712d1db`、`457081d8` | 已支持全选可编辑、反选可编辑、清空勾选和地图框选；框选只选择当前活动图层中可见且未锁定的要素，完成后退出框选模式，并通过覆盖层拦截鼠标事件，避免与 Mapbox Draw 拖拽编辑冲突。框选支持 Shift 追加、Alt/Option 减选；`Ctrl/Cmd+A` 可选择当前活动图层可编辑要素，`Escape` 可清空选择并退出框选，且输入框内不拦截快捷键。 |
